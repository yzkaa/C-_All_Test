...
void ImageWindow::init(){
    ...
    connect(ui->startButton,&QPushButton::clicked,this,&ImageWindow::startCounter);
}
...
void ImageWindow::mousePressEvent(QMouseEvent *e){
    startPos = ui->label->mapFromGlobal(e->globalPosition());
    endPos = startPos;
}
...
void ImageWindow::mouseMoveEvent(QMouseEvent *e){
    ...
    {
        endPos = ui->label->mapFromGlobal(e->globalPosition());
        this->repaint();
        //ui->label->setPixmap(image);
    }
...
void ImageWindow::saveImage(){
    ...
    ct->originImgPath = fileName;
    ct->labelImgPath = filename+"-副本.tif";
}

void ImageWindow::startCounter(){
    ct = new CounterThread(this);
    ct->start();
}
