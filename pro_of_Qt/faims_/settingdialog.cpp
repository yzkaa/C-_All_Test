    ...
    m_intValidator(new QIntValidator(0, 4000000, this))
{
    ...
    m_ui->autoCheckBox->setCheckable(true);
    ...
    connect(m_ui->autoCheckBox,SIGNAL(clicked()),m_timer,SLOT(start()));
...
void SettingDialog::fillPortsInfo()
{
    ...
    if(!list.empty()&&m_ui->autoCheckBox->isChecked()){
        emit ready();
    }
}
...
void SettingDialog::on_searchButton_clicked()
{
    fillPortsInfo();
}


void SettingDialog::Timer_Init(){
    m_timer=new QTimer();
    m_timer->stop();
    m_timer->setInterval(500);
    connect(m_timer,SIGNAL(timeout()),this,SLOT(on_searchButton_clicked()));
}
